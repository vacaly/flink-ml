...
public class FpTree {
    ...
    private FpTree project(int itemId, int minSupportCnt) {
        ...
        //p = summary.head;
        //while (p != null) {
        //    // trace upward again to set parent ptr of auxPtr
        //    Node f = p.parent;
        //    while (f != null) {
        //        if (f.parent != null) {
    ...
    private void extractImpl(int minSupportCnt, int item, int maxLength, int[] suffix,
        ...
        for (int i = 0; i < newSuffix.length; i++) {
            System.out.printf("%d,", newSuffix[i]);
        }
        System.out.printf("\ncount %d\n", summary.count);
        if (maxLength == 1) {
            return;
    ...
    public static void main(String[] args) {
        ...
        tree.createTree();
        tree.addTransaction(new int[] {5, 4, 1});
        tree.addTransaction(new int[] {5, 4, 3, 1});
        tree.addTransaction(new int[] {5, 4});
        tree.addTransaction(new int[] {5, 4, 1});
        tree.addTransaction(new int[] {5, 4, 3});
        tree.addTransaction(new int[] {5, 3});
        tree.initialize();
        tree.printProfile();
        tree.extractAll(new int[] {1}, 2, 4, null);
    }
}
