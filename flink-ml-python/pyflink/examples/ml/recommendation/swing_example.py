...
output = swing.transform(input_table)

# Extracts and display the results.
field_names = output[0].get_schema().get_field_names()

results = t_env.to_data_stream(
    output[0]).execute_and_collect()

for result in results:
    main_item = result[field_names.index(swing.get_item_col())]
    item_rank_score = result[1]
    print(f'main item: {main_item},\trecommended items: {item_rank_score}')