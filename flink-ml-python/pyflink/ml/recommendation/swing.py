...
class _SwingParams(
    ...
    MAX_USER_NUM_PER_ITEM: Param[int] = IntParam(
        ...
        "The max number of users that has purchased for each item. If the number of users "
        + "that have purchased this item is larger than this value, then only maxUserNumPerItem "
        + "users will be sampled and used in the computation logic.",
        1000,
        ParamValidators.gt(0))
...
class Swing(JavaRecommendationAlgoOperator, _SwingParams):
    ...
    Graph Construction for Recommendation in E-commerce" by Xiaoyong Yang, Yadong Zhu
    and Yi Zhang. (https://arxiv.org/pdf/2010.05525.pdf)
    """
